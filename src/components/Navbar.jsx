      ...
      <nav className="bg-[#0B0F19]/90 backdrop-blur-xl border-b border-white/5 px-4 md:px-8 py-4 flex justify-between items-center sticky top-0 z-[80]">
        ...
        <Link to="/" className="flex items-center gap-2 md:gap-3 shrink-0 group">
          ...
          <div className="flex flex-col">
            <span className="text-lg md:text-xl font-black text-white tracking-tighter leading-none italic uppercase">ShareHub</span>
            <span className="hidden xs:block text-[7px] md:text-[8px] font-black text-green-500 tracking-[0.2em] mt-1">CIRCULAR NETWORK</span>
          </div>
        </Link>

        {/* NAVEGACIÓN DESKTOP: Oculta en móviles pequeños */}
        ...
        <div className="flex items-center gap-3 md:gap-6">
          {!usuario ? (
            <div className="flex items-center gap-2 md:gap-4">
              <button onClick={() => setIsAuthOpen(true)} className="text-[9px] md:text-[10px] font-black text-slate-400 hover:text-white uppercase px-2 transition">
                Entrar
              </button>
              <button onClick={() => setIsAuthOpen(true)} className="bg-white text-black px-4 py-2 md:px-6 md:py-3 rounded-xl text-[9px] md:text-[10px] font-black uppercase hover:bg-green-500 transition-colors shadow-lg active:scale-95">
                Unirse
              </button>
            </div>
          ) : (
            <div className="flex items-center gap-3">
              <Link to="/garage" className="flex items-center gap-2 bg-white/5 border border-white/10 px-3 py-2 rounded-full hover:border-green-500/50 transition-all">
                <span className="text-base">{usuario.avatar || '👤'}</span>
                <span className="hidden sm:block text-[10px] font-black text-white uppercase">{usuario.name.split(' ')[0]}</span>
              </Link>
